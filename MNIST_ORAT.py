...
parser.add_argument('--epochs', type=int, default=50, metavar='N', help='number of epochs to train, 120 for WRN, 100 for resnet, 50 for lenet')
parser.add_argument('--weight_decay', '--wd', default=2e-4, type=float, metavar='W')
parser.add_argument('--lr', type=float, default=0.03, metavar='LR', help='learning rate')
parser.add_argument('--momentum', type=float, default=0.9, metavar='M', help='SGD momentum')
parser.add_argument('--epsilon', type=float, default=0.2, help='perturbation bound， 0.0078, 0.031 for CIFAR, 0.1, 0.3 for MNIST')
parser.add_argument('--num_steps', type=int, default=10, help='maximum perturbation step K')
parser.add_argument('--step_size', type=float, default=0.2/4, help='step size')
parser.add_argument('--seed', type=int, default=7, metavar='S', help='random seed')
parser.add_argument('--net', type=str, default="lenet_mnist",
...
parser.add_argument('--k', type=int, default=60000, help='k')
...
if 'cifar_10_noise' in args.dataset:
    ...
    train_loader = torch.utils.data.DataLoader(trainset, batch_size=128, shuffle=True)
    test_loader = torch.utils.data.DataLoader(testset, batch_size=128, shuffle=False)
    test_loader_aa = torch.utils.data.DataLoader(testset, batch_size=128, shuffle=False)
